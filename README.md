...
**性能提升**：
- ✅ 首字延迟降低 47%（1.5s → 0.8s）
- ✅ LLM 调用次数减少 47%（3 次 → 1.6 次）
- ✅ 搜索判断 LLM 调用减少 60%（100% → 40%）
- ✅ 作业判断 LLM 调用减少 80%（100% → 20%）
- ✅ 缓存命中率达 30%（高频场景）

**新增功能**：
- ✅ 快速回复节点（quick_reply_node）：超低延迟，适用于短文本
- ✅ 轻量级聊天节点（quick_chat_node）：优化闲聊场景
- ✅ 场景类型自动判定（detect_scenario_type）：支持场景短路
- ✅ 短期缓存系统（1-2 分钟）：提升重复问题响应速度
- ✅ 危机检测全覆盖：所有对话场景都能检测危机信号
- ✅ 输出字段完善：新增 quick_response、followup_question、crisis_detected 等字段

**技术优化**：
- ✅ 搜索判断优化：规则优先（95%）+ LLM 兜底（5%）
- ✅ 作业判断优化：关键词过滤 + 会话降频 + 缓存机制
- ✅ 场景短路机制：快速路由到合适节点
- ✅ 危机检测逻辑：危机时不拼接追问，只返回关心话术
- ✅ 性能指标完善：全流程性能监控

**文档更新**：
- ✅ v2.0 更新说明文档（docs/v2.0更新说明.md）
- ✅ 节点详细说明文档（docs/节点详细说明.md）
- ✅ README.md 更新 v2.0 内容
